...
@ torch.no_grad()
def generate(model: Any, prompt: torch.Tensor, attention_mask: torch.Tensor | None = None, steps=128, gen_length=128, block_length=128, temperature=0.,
             cfg_scale=0., remasking='low_confidence', mask_id=126336, logits_eos_inf=False, confidence_eos_eot_inf=False,
             record_history: bool = False, history_stride: int = 1) -> tuple[torch.Tensor, list[torch.Tensor]]:
    '''
    Args:
    ...
    for num_block in range(num_blocks):
        ...
        for i in range(steps):
            ...
            for j in range(confidence.shape[0]):
                ...
                # (optional safety) cap to valid range
                k = min(k, confidence[j].numel())

                # Select the k highest-confidence masked positions for this batch element
                _, select_index = torch.topk(confidence[j], k=k)
                transfer_index[j, select_index] = True

            # Apply the selected updates for this step
