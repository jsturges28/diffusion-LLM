...
- [x] Analytics **Entropy by Position** chart: the first chart indexed by token position instead of frame (bars, on the Entropy overlay's ramp, hover lighting the column and naming the token), with edit-orange markers and tint at edited positions, a divergence-aware tooltip that splits into Original and Edited rows where a What If branch stops sharing its prefix, and an Original/Edited crossfade slider over the two runs' bars; restores a meaningful third chart for autoregressive runs, whose per-frame mean is a cumulative average and therefore flat by construction
